import { PeriodResultPresentionItem, YearResultPresentationItem } from "@/types";
import api from "../fetcher";

export async function getPeriodResultPresentation(searchParams: {
  yearId: number;
  facultyId: number;
  departmentId: number;
  classId: number;
  ...
  return res.data as PeriodResultPresentionItem[];
}

export async function getYearResultPresentation(searchParams: {
  yearId: number;
  facultyId: number;
  departmentId: number;
  classId: number;
  ...
  query.append("mode", mode.toString());
  const res = await api.get(`/jury/result-presentation/?${query.toString()}`);
  return res.data as YearResultPresentationItem[];
}
